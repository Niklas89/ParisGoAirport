...
if(isset($_POST['submit']))  {
  if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['arrivaldate'])  && !empty($_POST['arrivaltime'])   && !empty($_POST['email'])  && !empty($_POST['phone']) && !empty($_POST['shuttlefee'])
    ...
    $valid = true;
     ...
     $valid = false;
    $erreurid = 'Please fill in all the fields. <a href="reservation/reservationway-other.php" title="Reservation">Reservation page</a>';
    }
    ...
    if($valid)
    {
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,email,phone,nbadults,nbchildren,shuservice,parisaddress,shuttlefee,way,returningdate,returningtime,location) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:email,:phone,:nbadults,:nbchildren,:shuservice,:parisaddress,:shuttlefee,:way,:returningdate,:returningtime,:location)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $sth = $bdd->prepare("SELECT id FROM personnes WHERE coldate = :coldate");
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
            ...
            try {
              $mailmessage = file_get_contents('reservation/round-trip-other-mail.html');
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,email,phone,nbadults,nbchildren,shuservice,parisaddress,shuttlefee,way,location) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:email,:phone,:nbadults,:nbchildren,:shuservice,:parisaddress,:shuttlefee,:way,:location)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $sth = $bdd->prepare("SELECT id FROM personnes WHERE coldate = :coldate");
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
            ...
            try {
              $mailmessage = file_get_contents('reservation/one-way-other-mail.html');
              $mailmessage = str_replace('%firstname%', $firstname, $mailmessage);  
              $mailmessage = str_replace('%lastname%', $lastname, $mailmessage); 
              $mailmessage = str_replace('%arrivaldate%', $arrivaldate, $mailmessage); 
              $mailmessage = str_replace('%arrivaltime%', $arrivaltime, $mailmessage); 
