...
if(isset($_POST['submit']))  {
  ...
  if($way == '单程') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "姓（拼音）") && !empty($_POST['lastname']) && ($_POST['lastname'] != "姓（拼音）") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "离港日期")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "离港时间") 
               && !empty($_POST['email']) && ($_POST['email'] != "电子邮件")  && !empty($_POST['phone']) && ($_POST['phone'] != "电话号码（附加国家区号)")
                 && !empty($_POST['location']) && ($_POST['location'] != "目的地(法国名)")
             && ($_POST['nbadults'] != "人数（3岁以上)") && !empty($_POST['parisaddress']) && ($_POST['parisaddress'] != "巴黎地址")
              && ($_POST['nbchildren'] != "婴儿座椅") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
        ...
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "姓（拼音）") && !empty($_POST['lastname']) && ($_POST['lastname'] != "姓（拼音）") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "离港日期")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "离港时间") 
           && ($_POST['returningdate'] != "返回日期")  && !empty($_POST['returningtime']) && ($_POST['returningtime'] != "返回时间")
               && !empty($_POST['email']) && ($_POST['email'] != "电子邮件")  && !empty($_POST['phone']) && ($_POST['phone'] != "电话号码（附加国家区号)")
                 && !empty($_POST['location']) && ($_POST['location'] != "目的地(法国名)")
            && !empty($_POST['parisaddress']) && ($_POST['parisaddress'] != "巴黎地址") && ($_POST['nbadults'] != "人数（3岁以上)")
              && ($_POST['nbchildren'] != "婴儿座椅") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
               && !empty($_POST['num2']) && !empty($_POST['captcha']) && ($_POST['captcha'] != "有效验证")) {
                extract($_POST);
                $num1 = $_POST['num1'];
