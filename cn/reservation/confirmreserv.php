  ...
  if($way == '单程') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "姓（拼音）") && !empty($_POST['lastname']) && ($_POST['lastname'] != "名（拼音）") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "到港日期/离港日期")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "到港时间/离港时间")   
          && !empty($_POST['flightnumb']) && ($_POST['flightnumb'] != "航班号")   && !empty($_POST['airport']) && ($_POST['airport'] != "飞机场")
             && !empty($_POST['parisaddress']) && ($_POST['parisaddress'] != "巴黎地址") && !empty($_POST['terminal']) && ($_POST['terminal'] != "航站楼")
               && !empty($_POST['email']) && ($_POST['email'] != "电子邮件")  && !empty($_POST['phone']) && ($_POST['phone'] != "电话号码（附加国家区号)")
            && ($_POST['tfairport'] != "送机或接机")  && ($_POST['shuservice'] != "请选择服务") && ($_POST['nbadults'] != "人数（3岁以上)")
    ...
    if($way == '往返') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "姓（拼音）") && !empty($_POST['lastname']) && ($_POST['lastname'] != "名（拼音）") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "到港日期/离港日期")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "A到港时间/离港时间") 
