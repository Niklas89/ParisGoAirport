...
if(isset($_POST['submit']))  {
  if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['arrivaldate'])  && !empty($_POST['arrivaltime'])   && !empty($_POST['email'])  && !empty($_POST['phone']) && !empty($_POST['shuttlefee'])
    ...
    $valid = true;
     ...
     $valid = false;
    $erreurid = 'Please fill in all the fields. <a href="reservation/reservationway-disney.php" title="Reservation">Reservation page</a>';
    }
    ...
    if($valid)
    {
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,flightnumb,airport,terminal,email,phone,tfairport,nbadults,nbchildren,shuservice,parisaddress,shuttlefee,way,returningdate,returningtime,location) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:flightnumb,:airport,:terminal,:email,:phone,:tfairport,:nbadults,:nbchildren,:shuservice,:parisaddress,:shuttlefee,:way,:returningdate,:returningtime,:location)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $sth = $bdd->prepare("SELECT id FROM personnes WHERE coldate = :coldate");
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
                            ...
                            <input type="hidden" name="business" id="business" value="booking@user.com">
                            <input type="hidden" name="currency_code" value="EUR">
                            <INPUT type="hidden" name="charset" value="utf-8">
                            <input name="cancel_return" type="hidden" value="http://www.parisgoairport.com" />
                            <input name="no_note" type="hidden" value="1" />
                            <input name="bn" type="hidden" value="PP-BuyNowBF" />
                            <input type="hidden" name="item_name" id="item_name" value="'.$arrivaldate.' Round-Trip '.$nbadults.'pax">
            ...
            try {
              $mailmessage = file_get_contents('reservation/round-trip-disney-mail.html');
              ...
              $mailmessage = str_replace('%location%', $location, $mailmessage);
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,flightnumb,airport,terminal,email,phone,tfairport,nbadults,nbchildren,shuservice,parisaddress,shuttlefee,way,location) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:flightnumb,:airport,:terminal,:email,:phone,:tfairport,:nbadults,:nbchildren,:shuservice,:parisaddress,:shuttlefee,:way,:location)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $sth = $bdd->prepare("SELECT id FROM personnes WHERE coldate = :coldate");
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
                            ...
                            <input name="no_note" type="hidden" value="1" />
                            <input name="bn" type="hidden" value="PP-BuyNowBF" />
                            <input type="hidden" name="item_name" id="item_name" value="'.$arrivaldate.' One-Way '.$nbadults.'pax">
            ...
            try {
              $mailmessage = file_get_contents('reservation/one-way-disney-mail.html');
              $mailmessage = str_replace('%firstname%', $firstname, $mailmessage); 
              $mailmessage = str_replace('%lastname%', $lastname, $mailmessage); 
              $mailmessage = str_replace('%arrivaldate%', $arrivaldate, $mailmessage); 
              $mailmessage = str_replace('%arrivaltime%', $arrivaltime, $mailmessage); 
