...
if(isset($_POST['submit']))  {
  if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['arrivaldate'])  && !empty($_POST['arrivaltime'])   && !empty($_POST['email'])  && !empty($_POST['phone']) && !empty($_POST['shuttlefee'])
    ...
    $valid = true;
     ...
     $valid = false;
    $erreurid = 'Please fill in all the fields. <a href="reservation/reservationway-airp.php" title="Reservation">Reservation page</a>';
    }
    ...
    if($valid)
    {
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,flightnumb,terminal,email,phone,tfairport,nbadults,shuservice,shuttlefee,way,returningdate,returningtime) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:flightnumb,:terminal,:email,:phone,:tfairport,:nbadults,:shuservice,:shuttlefee,:way,:returningdate,:returningtime)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $ok = '
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
            ...
            try {
              $mailmessage = file_get_contents('reservation/round-trip-airp-mail.html');
      ...
      $req = $bdd->prepare('INSERT INTO personnes (coldate,firstname,lastname,arrivaldate,arrivaltime,flightnumb,terminal,email,phone,tfairport,nbadults,shuservice,shuttlefee,way) 
        VALUES (:coldate,:firstname,:lastname,:arrivaldate,:arrivaltime,:flightnumb,:terminal,:email,:phone,:tfairport,:nbadults,:shuservice,:shuttlefee,:way)');
      $req->execute(array(
        'coldate'=>$coldate,
        'firstname'=>$firstname,
        'lastname'=>$lastname,
      ...
      $sth = $bdd->prepare("SELECT id FROM personnes WHERE coldate = :coldate");
       ...
       <img class="pp-img" src="../img/paypal.png" alt="Make payments with PayPal - it\'s fast, free and secure!" title="PayPal" />
            ...
            try {
              $mailmessage = file_get_contents('reservation/one-way-airp-mail.html');
              $mailmessage = str_replace('%firstname%', $firstname, $mailmessage); 
              $mailmessage = str_replace('%lastname%', $lastname, $mailmessage); 
              $mailmessage = str_replace('%arrivaldate%', $arrivaldate, $mailmessage); 
              $mailmessage = str_replace('%arrivaltime%', $arrivaltime, $mailmessage); 
