  ...
  if($way == 'One-Way') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "Firstname") && !empty($_POST['lastname']) && ($_POST['lastname'] != "Lastname") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "Arrival/Departing Date")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "Arrival/Departing Time") 
            ...
            && ($_POST['tfairport'] != "To or From airport ?") && ($_POST['nbadults'] != "Adults (over 3 years old)")
              && ($_POST['nbchildren'] != "Children (under 3 years old)") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
               && !empty($_POST['num2']) && !empty($_POST['captcha']) && ($_POST['captcha'] != "Human ?")) {
                ...
                if( intval($num1) + intval($num2) == intval($total) ) {
                          ...
                          header('Location: one-way-reservation-disney.php?way='.urlencode($way).'&captcha=false');
                  }
          
          } else {
           $valid = false;
          
           header('Location: one-way-reservation-disney.php?way='.urlencode($way).'&notvalid=false');
              ...
              && ($_POST['nbchildren'] != "Children (under 3 years old)") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
               && !empty($_POST['num2']) && !empty($_POST['captcha']) && ($_POST['captcha'] != "Human ?")) {
                ...
                if( intval($num1) + intval($num2) == intval($total) ) {
                          ...
                          header('Location: round-trip-reservation-disney.php?way='.urlencode($way).'&captcha=false');
                  }
          
          } else {
           $valid = false;
          
           header('Location: round-trip-reservation-disney.php?way='.urlencode($way).'&notvalid=false');
  ...
  if($valid)
  {
   // header("confirmreserv.php");
    ...
    $coldate = date('Y-m-d H:i:s');
      ...
      if($terminal == 'Airport Terminal (optional)') {
        $terminal = '';
      }
      if($flightnumb == 'Flight Number (optional)') {
        $flightnumb = '';
      }
      if($parisaddress == 'Paris Address (optional)') {
        $parisaddress = '';
      }
        ...
        <div class="main-container">
            <div class="main wrapper clearfix">
              <div class="paddingdiv">
                ...
                <?php if( $location == 'Paris address') { ?> 

                        <h3 class="title-prices-second"><?php echo $way; ?> <?php echo $tfairport; ?> <?php echo $airport; ?></h3>
                    <table>
                       <tr>
                           <th>Departing date from Paris or Disney</th>
                           <td><?php echo $arrivaldate; ?></td>
                       </tr>
                       <tr>
                           <th>Departing time from Paris or Disney</th>
                    ...
                    <table>
                       ...
                       <tr>
                           <th>Flight arrival date / departing date from Disney</th>
                           <td><?php echo $arrivaldate; ?></td>
                       </tr>
                       <tr>
                           <th>Flight arrival time / departing time from Disney</th>
                           <td><?php echo $arrivaltime; ?></td>
                       </tr><?php if($way == 'Round-Trip') { ?>
                       <tr>
                           <th>Returning Date</th>
                    ...
                    if ($way == "Round-Trip") {
                     ...
                     <form class="login-form" id="SignupForm" action="../tours-disney-res.php" method="post">
