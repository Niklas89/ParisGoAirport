...
if(isset($_POST['submit']))  {
  ...
  if($way == 'One-Way') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "Firstname") && !empty($_POST['lastname']) && ($_POST['lastname'] != "Lastname") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "Date")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "Time") 
               && !empty($_POST['email']) && ($_POST['email'] != "Email")  && !empty($_POST['phone']) && ($_POST['phone'] != "Phone with country code")
                 && !empty($_POST['location']) && ($_POST['location'] != "Destination")
             && ($_POST['nbadults'] != "Adults (over 3 years old)") && !empty($_POST['parisaddress']) && ($_POST['parisaddress'] != "Paris Address")
              && ($_POST['nbchildren'] != "Children (under 3 years old)") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
               && !empty($_POST['num2']) && !empty($_POST['captcha']) && ($_POST['captcha'] != "Human ?")) {
                ...
                if( intval($num1) + intval($num2) == intval($total) ) {
                          ...
                          header('Location: one-way-reservation-other.php?captcha=false');
                  }
          
          } else {
           $valid = false;
          
           header('Location: one-way-reservation-other.php?notvalid=false');
    ...
    if($way == 'Round-Trip') {
        if(!empty($_POST['firstname']) && ($_POST['firstname'] != "Firstname") && !empty($_POST['lastname']) && ($_POST['lastname'] != "Lastname") && !empty($_POST['arrivaldate'])
         && ($_POST['arrivaldate'] != "Departing Date")  && !empty($_POST['arrivaltime']) && ($_POST['arrivaltime'] != "Departing Time") 
           && ($_POST['returningdate'] != "Returning Date")  && !empty($_POST['returningtime']) && ($_POST['returningtime'] != "Returning Time")
               && !empty($_POST['email']) && ($_POST['email'] != "Email")  && !empty($_POST['phone']) && ($_POST['phone'] != "Phone with country code")
                 && !empty($_POST['location']) && ($_POST['location'] != "Location")
            && !empty($_POST['parisaddress']) && ($_POST['parisaddress'] != "Paris Address") && ($_POST['nbadults'] != "Adults (over 3 years old)")
              && ($_POST['nbchildren'] != "Children (under 3 years old)") && preg_match('#^[a-z0-9.!\#$%&\'*+-/=?^_`{|}~]+@([0-9.]+|([^\s]+\.+[a-z]{2,6}))$#si', $_POST['email']) && !empty($_POST['num1'])
               && !empty($_POST['num2']) && !empty($_POST['captcha']) && ($_POST['captcha'] != "Human ?")) {
                extract($_POST);
                $num1 = $_POST['num1'];
                ...
                if( intval($num1) + intval($num2) == intval($total) ) {
                          ...
                          header('Location: round-trip-reservation-other.php?captcha=false');
                  }
          
          } else {
           $valid = false;
          
           header('Location: round-trip-reservation-other.php?notvalid=false');
                ...
                <p class="main-description">Confirm that the details below are correct. <a href="shuttle-prices.php" title="Prices" target="_blank">Shuttle prices</a></p>
                    ...
                    elseif(isset($erreurid)){ ?>
                      ...
                      <?php echo '<p id="erreurid">'.$erreurid.'</p>'; } ?>
                        ...
                        <h3 class="title-prices-second"><?php echo $way; ?> <?php echo $location; ?> </h3>
                    <table>
                       <tr>
                    ...
                    if ($way == "One-Way") {
                      ...
                      if($location == "Parc des Expositions") {
                      ...
                      } // end if Parc des Expositions

                      if($location == "Gare du Nord" || $location == "Gare de l'Est" || $location == "Saint-Lazare" || $location == "Gare d'Austerlitz"
                       || $location == "Gare Montparnasse" || $location == "Gare de Lyon" || $location == "Gare de Bercy" || $location == "Gare dAusterlitz" || $location == "Gare de lEst") {
                      ...
                      } // end if Train stations

                      if($location == "Fontainebleau" || $location == "Barbizon" || $location == "Giverny") {

                        if($nbadults == "1"){
                             $shuttlefee="120"; echo "120";
                          }
                        if($nbadults == "2"){
                            $shuttlefee="120"; echo "120";
                          }
                      ...
                      } // end if fontainebleau

                      if($location == "Eiffel Tower" || $location == "Tour Eiffel" || $location == "Montmartre" || $location == "Louvre"
                       || $location == "Notre-Dame" || $location == "Beaubourg" || $location == "Grand Palais" || $location == "Arc de Triomphe") {
                    ...
                    if ($way == "Round-Trip") {
                     ...
                     <form class="login-form" id="SignupForm" action="../tours-other-res.php" method="post">
